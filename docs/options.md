...
| `--ld`      | LD file prefix for each cohort `.bim .ld (.frq)`<br>The `.ld` file must contain exactly three whitespace-delimited columns in the order `SNP1 SNP2 R` (no header). |
